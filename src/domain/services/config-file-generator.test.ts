...
describe("config-file-generator", () => {
  ...
  describe("generateConfigFileContent", () => {
    ...
    test("generates correct YAML content", () => {
      ...
      // Check for yaml-language-server schema comment
    ...
    test("generates YAML without {} for empty config", () => {
      const content = generateConfigFileContent({}, "yaml");

      // Should only contain the schema comment
      expect(content).toBe(
        "# yaml-language-server: $schema=https://unpkg.com/mermaid-markdown-wrap/schema/config.schema.json\n",
      );
